...
Na czym on polega? Wydaje mi się że wyłączenie nauronu to po prostu nałożenie na niego funkcji aktywacyjnej które niezależnie od wejścia zwraca zero. Funkcja taka ma zerowy gradient, więc jeżeli robimy wsteczną propagację to nie będziemy modyfikowali wag "wyłączonego" nauronu.
...
Przepuszczam obraz przez sieć po czym dostaję predykcję bounding boxów i class elementów w bounding boxach.

Następnie stosuje algorytm NMS. Ma 2 thresholdy. Jeden usuwa boxy które mają małe prawdopodobieństwa wystąpienia obiektu w boxie (niskie pc). Drugi odpowiada za IoU. Jeżeli IoU wynosi *n* to elementy z tej samej klasy dla których bounding boxy pokrywają się w takim stopniu że IoU jest większy nić \*n\* to usuwamy je (z wyjątkiem tego o największym Pc).
...
Transformery nie są rekurencyjne. Zamiast rekurencyjności dodajemy informacje o pozycjach elementów (np. wyrazów w zdaniu).

Czy transformery są obliczane autoregresyjnie?. Przy inferencji musi tak być (tak, jak przy RNN). Przy trenowaniu, może, ale z przyczyn optymalizacji obliczeń się tego nie stosuje. Żeby sieć nie znała w czasie treningu przyszłosci stosuje się maski.

[Is the Transformer decoder an autoregressive model?](https://datascience.stackexchange.com/questions/104179/is-the-transformer-decoder-an-autoregressive-model#:~:text=As%20a%20whole%2C%20the%20Transformer,of%2Dthe%2Dbox%20solutions.)

Sposoby optymalizacji transformerów: [Large Transformer Model Inference Optimization](https://lilianweng.github.io/posts/2023-01-10-inference-optimization/)
...
Dostaje obserwacje na postawie calego zdanie. Zdanie to dekomponuje na macierz o wymiarach vxd. v - to slownik (wszystkie mozliwe unikalne wyrazy we wszystkich probkach) a d to ilosc slow. Nastepnie z mojego zdania, slowo po slowie laduje do sieci i dostaje kolejne wyniki o wymiarach e (e to wymiar embeddingu).
