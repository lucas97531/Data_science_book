...
Mamy szereg DR (Default Rate) miesiąc po miesiącu. DR jest liczony w okresie LIP (okres od zdarzenia, czyli np. utrata pracy, do defaultu kredytu powodowanego przez to zdzarzenie). Czyli dla stycznia mamy styczniowe kredyty i badamy ile proc. z nich wpadło w default. I tak dalej. Problem jest tutaj taki, że w naszym szeregu DR-ów mamy silną autokorelację. Defaultu z okien czasowych (długości LIP), zazębiają się dla poszczególnych miesięcy. Czyli w przypadku LIP=3miesiące dla DR styczniowe i lutowego nakładają się okresy luty i marze okien czasowych. To można rozwiązać na 3 sposoby:
