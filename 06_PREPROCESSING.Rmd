...
3.  Obliczamy REACHABILITY DENSITY (RD) dla każdej pary punktów. UWAGA: ta miara nie jest symetryczna, więc RD z A do B to nie to samo co RD z B do A. Jeżeli odległość punktu od punktu analizowanego jest w promieniu wyznaczonym przez najdalszego sąsiada to RD równa się temu promieniowi. Jeżeli jest poza promieniem to RD jest równe odległości pomiędzy punktami. Popatrzmy na przykład:

    ![](06_PREPROCESSING/figures/LOF_2.png)

    Mamy dwa punkty Xj i Xi i chcemy policzyć RD z Xj do Xi. Rozpatrujemy 2 możliwe położenia punktu Xi. W pierwszym jest wewnątrz koła wyznaczonego przez k-tych najdalszych sąsiadów. Jego RD jest równe niebieskiej linii, czyli promienia okręgu wyznaczonego przez najdalszych sąsiadów. W drugim przypadku jest poza kołem i jego RD jest równe odległości od Xj (pomarańczowa linia).
...
Obliczny dystans dla każdego puntu do k-tych najbliższych sąsiadów:
...
Uwaga. W przypadku klasyfikacji terminy *imbalanced data* i *skewed data* są najczęściej używane zamiennie.
...
-   Determine which class in your dataset is underrepresented (the minority class) and which is overrepresented (the majority class).

------------------------------------------------------------------------

**2. Select Minority Class Samples**

-   Randomly choose a sample from the minority class as the base (seed) sample for generating synthetic data.

------------------------------------------------------------------------

**3. Find Nearest Neighbors**

-   Identify the k-nearest neighbors (typically k=5) of the selected sample in the feature space. These are the kkk samples closest to the selected sample, based on a distance metric (e.g., Euclidean distance).

------------------------------------------------------------------------

**4. Create Synthetic Samples**

-   For each selected sample $x$:

    -   Randomly choose one of its nearest neighbors $x_{\text{neighbor}}$.

    -   Create a new synthetic sample $x_{\text{synthetic}}$ by interpolating between $x$ and $x_{\text{neighbor}}$: $$
        x_{\text{synthetic}} = x + \delta \cdot (x_{\text{neighbor}} - x)
        $$

        -   Here, $\delta$ is a random number between 0 and 1, ensuring the synthetic sample lies on the line segment between $x$ and $x_{\text{neighbor}}$.

------------------------------------------------------------------------

**5. Repeat Until the Desired Balance is Achieved**

-   Continue generating synthetic samples until the minority class has enough samples to match or reach a desired proportion of the majority class.
...
3.  **Computational Cost**: Finding kkk-nearest neighbors can be computationally expensive for large datasets.
...
**TF-IDF (Term Frequency-Inverse Document Frequency)** is a statistical measure used to evaluate the importance of a word in a document relative to a collection (or corpus) of documents. It combines two key metrics:
...
If a term appears in many documents, its IDF value will be lower, as it is considered less informative.

------------------------------------------------------------------------

3\. TF-IDF Score

TF-IDF combines the two metrics to assign a weight to each term:
...
1.  **Tokenize the Text**: Split the document and corpus into individual terms (words or tokens).
2.  **Calculate TF for Each Term**: Determine the frequency of each term in a specific document.
3.  **Calculate IDF for Each Term**: Determine how many documents in the corpus contain the term and compute the inverse document frequency.
4.  **Compute TF-IDF**: Multiply the TF and IDF values for each term in the document.
5.  **Rank Terms**: The resulting TF-IDF scores indicate the terms most relevant to the document compared to the corpus.
...
-   **TF in Document 1:** $$
    \text{TF} = \frac{1}{6} = 0.167
    $$

-   **IDF Across Both Documents:** Only Document 1 contains the term "cat," so: $$
    \text{IDF} = \log \left( \frac{2}{1} \right) = 0.301
    $$

-   **TF-IDF for "cat" in Document 1:** $$
    \text{TF-IDF} = 0.167 \times 0.301 = 0.050
    $$
...
-   **Search Engines**: Identifying keywords to improve relevance ranking.
-   **Text Summarization**: Highlighting important terms in a document.
-   **Recommendation Systems**: Identifying shared preferences based on textual data.
...
opis algoytmu: [link](https://towardsdatascience.com/lines-detection-with-hough-transform-84020b3b1549)
